class Api::SessionsController < Devise::SessionsController
  ...
  def create
    ...
    id = Usuario.find_by(email: email).try(:id) if email.presence
    ...
    if user
      if user.valid_password? password
        ...
        render status: 200, json: { email: user.email, authentication_token: user.authentication_token, id: id, perfil_id: user.perfil_id}
      else
