		...
		if params[:zona] != "" && params[:lector] != ""		
		 ...
		 if !listaAsignaciones.blank?
			 listaAsignaciones.each do |asignacion|
				 ...
				 problemasTecnicos = Asignacion.where(empleado_id: asignacion.empleado_id, rutum_id: asignacion.rutum_id).joins(:orden_lectura => [:detalle_orden_lectura => :clave_lectura]).where(orden_lecturas: {estado_lectura_id: [4,5], relectura:false}, clave_lecturas: {id: [16,17,19]}).count
	 	...
	 	listaEmpleados.each do |empleado|
			...
			if !listaAsignaciones.blank?
				listaAsignaciones.each do |asignacion|
					ruta = asignacion.rutum
				 	comuna = asignacion.orden_lectura.comuna.nombre
				  asignado = Asignacion.where(empleado_id: asignacion.empleado_id, rutum_id: asignacion.rutum_id).joins(:orden_lectura => [:detalle_orden_lectura]).where(orden_lecturas: {estado_lectura_id: [2,3], relectura: false}).count
